...
# Deploys Unix workload (FreeBSD/RHEL) to isolated production account
...
terraform {
  ...
  backend "s3" {
    ...
    # key    = "landing-zone/environments/prod/terraform.tfstate"
    # region = "us-west-2"
  }
}
...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::${var.prod_account_id}:role/CrossAccountAdminRole"
  }
}
